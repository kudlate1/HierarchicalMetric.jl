function LAT_vec(init::InitCenters, X::Matrix, k::Int; h::Real=2.0, λ::Real=0.8, max_iter::Int=200)
    d, n = size(X)
    ...
    for iter in 1:max_iter
        ...
        for j in 1:100
            ...
            (y_n == 0) && return c, ones(Int64, n), [w * w' for w in W], probs, history, iters
            state_tree = Flux.setup(opt, W)
            loss, grad = Flux.withgradient(W) do w
                triplet_loss_vec(anchor, pos, neg, y_p, y_n, w; α = 1.5, λₗₐₛₛₒ = 1.1)
            end
...
function LAT_htd(init::InitCenters, X, k::Int; h::Real=2.0, λ::Real=0.1, max_iter::Int=200)
    ...
    for iter in 1:max_iter
        ...
        #println("Iter $iter, w1 = $(old_params[1][1]), w2 = $(old_params[1][2])")
        # 2. E-step: compute responsibilities and cluster points
        ...
        for j in 1:10
            ...
            (y_n == 0) && return c, ones(Int64, n), W, history, iters
            state_tree = Flux.setup(opt, W)
            loss, grad = Flux.withgradient(W) do w
                triplet_loss_htd(anchor, pos, neg, y_p, y_n, w; α = 0.3, λₗₐₛₛₒ = 0.01, weight_transform=softplus)
            end
            Flux.update!(state_tree, W, grad[1]) 
        end
